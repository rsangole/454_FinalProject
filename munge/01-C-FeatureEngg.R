...
# PCA
## PCA for all components except Soil Type & Wilderness Type
...
## PCA for Soil Type & Wild Type
pca_fit2 <- prcomp(x = raw_train %>% select(contains('wild'),contains('soil')), center = T, scale. = T)
pca_fit2
pca_fit2 %>% summary
...
add_pca_transform_soil_wild <- function(df){
    ...
    pca_df <- as_tibble(predict(pca_fit2, raw_train %>% select(contains('wild'),contains('soil')))[,1:4])
    keep = !(names(df) %in% names(df %>% dplyr::select(contains('wild'),contains('soil'))))
    pca_df %>% bind_cols(df[keep])
}
add_pca_transform_soil_wild(raw_train)
